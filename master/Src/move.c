...
extern Ramp_Typedef Step_Ramp;										//原地踏步Ramp
...
void Robot_Move_Jump(Robot_Handle *Robot,Leg_Handle *Leg,Dev_Handle *IMU){
	static float ready_y = -60.0f;								//准备起跳位置y						//一下几个系数均有关系
	static float ready_z = 150.0f;								//准备起跳位置z
	static float jump_up_y	= -60.0f;							//起跳位置y
	static float jump_up_z  = -150.0f;						//起跳位置z
	static float jump_up_back_y = 120.0f;					//起跳后腿收回
	static float jump_up_back_z = 150.0f;					//跳跃后腿收回
	static float jump_down_buffer_y = 120.0f;			//跳跃缓冲y
	static float jump_down_buffer_z = -150.0f;		//跳跃缓冲z
	static float jump_back_y = -120.0f;						//返回原位
	static float jump_back_z = 0.0f;							//返回原位
