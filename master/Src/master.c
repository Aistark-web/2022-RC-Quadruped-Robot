...
Ramp_Typedef Seesaw_Sqat_Ramp[4];							//跷跷板蹲下Ramp
Ramp_Typedef Seesaw_Sqat_Back_Ramp[4];					//跷跷板蹲下返回Ramp
Ramp_Typedef Seesaw_Climb_Up_Posture_Ramp[4];				//跷跷板上坡姿态Ramp
Ramp_Typedef Seesaw_Climb_Down_Posture_Ramp[4];			//跷跷板下坡姿态Ramp
Ramp_Typedef Seesaw_Climb_Buffer_Ramp;							//攀爬缓冲Ramp
Ramp_Typedef Seesaw_Sqat_Buffer_Ramp;								//蹲下缓冲
...
void Robot_init()
{
	...
	for(uint8_t i = 0;i<4;i++){
		...
		/* 跷跷板 */
		Seesaw_Sqat_Ramp[i].RampTime								=	300;	//跷跷板蹲下Ramp
		Seesaw_Sqat_Back_Ramp[i].RampTime						=	300;	//跷跷板蹲下返回Ramp
